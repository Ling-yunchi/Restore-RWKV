...
def synthesisOneAxial(model, img, kernel_size=64, stride=32, crop_size=3):
    ...
    for k in range(nz):
        ...
        if idz + crop_size + stride == D:
            ...
            flag = False
        x = img[:, :, k * stride:k * stride + kernel_size, :, :] if flag else img[:, :, D - kernel_size:, :,
                                                                              :]  ##Large patches along z axis
        ...
        for i in range(nx):
            ...
            for j in range(ny):
                ...
                if flag:
                    result[:, :, idz:k * stride + kernel_size, idx:i * stride + kernel_size,
                    idy:j * stride + kernel_size] = G_patches[i * nx + j, 0, idz - k * stride:, idx - i * stride:,
                                                    idy - j * stride:].unsqueeze(0).unsqueeze(0)
                else:
                    result[:, :, idz:, idx:i * stride + kernel_size, idy:j * stride + kernel_size] = G_patches[
                                                                                                     i * nx + j, 0,
                                                                                                     idz + kernel_size - D:,
                                                                                                     idx - i * stride:,
                                                                                                     idy - j * stride:].unsqueeze(
                        0).unsqueeze(0)
    return result
