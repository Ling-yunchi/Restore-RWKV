...
class Routing_Module(nn.Module):
    def __init__(self, global_dim, hidden_dim, num_expters):
        ...
        self.conv1 = nn.Conv2d(in_channels=global_dim + hidden_dim, out_channels=hidden_dim, kernel_size=1, padding=0,
                               stride=1, groups=1, bias=True)

        self.gelu = nn.GELU()
        self.conv3 = nn.Conv2d(in_channels=hidden_dim, out_channels=num_expters, kernel_size=1, padding=0, stride=1,
                               groups=1, bias=True)
        self.relu = nn.ReLU()
...
class Attention_Bank(nn.Module):
    def __init__(self, n_feats, ch_exp_list=[1, 2, 3], reduction=16):
        ...
        self.att_experts = nn.ModuleList(
            [Attention_Expert(n_feats, ch_exp_f=ch_exp_list[i], reduction=reduction) for i in range(self.num_expters)])
...
class MLP_Bank(nn.Module):
    def __init__(self, n_feats, mlp_ratio_list=[1, 2, 4]):
        ...
        self.mpl_experts = nn.ModuleList(
            [MLP_Expert(in_feat=n_feats, h_feat=int(mlp_ratio_list[i] * n_feats), out_feat=n_feats) for i in
             range(self.num_expters)])
