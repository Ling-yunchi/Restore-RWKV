...
class Restore_RWKV(nn.Module):
    def __init__(self,
        ...
        dim: int, number of channels in the model
        num_blocks: list number of blocks in each level
        num_refinement_blocks: int, number of refinement blocks
        add_raw: bool, whether to add the input image to the output image, need inp_channels == out_channels
        """
        super(Restore_RWKV, self).__init__()
        self.add_raw = add_raw
        assert not add_raw or inp_channels == out_channels, \
            "If add_raw is True, inp_channels should be equal to out_channels."
        ...
        self.encoder_level2 = nn.Sequential(
            *[Block(n_embd=int(dim * 2 ** 1), n_layer=num_blocks[1], layer_id=i) for i in range(num_blocks[1])])

        self.down2_3 = Downsample(int(dim * 2 ** 1))  # From Level 2 to Level 3
        self.encoder_level3 = nn.Sequential(
            *[Block(n_embd=int(dim * 2 ** 2), n_layer=num_blocks[2], layer_id=i) for i in range(num_blocks[2])])

        self.down3_4 = Downsample(int(dim * 2 ** 2))  # From Level 3 to Level 4
        self.latent = nn.Sequential(
            *[Block(n_embd=int(dim * 2 ** 3), n_layer=num_blocks[3], layer_id=i) for i in range(num_blocks[3])])
        ...
        self.decoder_level1 = nn.Sequential(
            *[Block(n_embd=int(dim * 2 ** 1), n_layer=num_blocks[0], layer_id=i) for i in range(num_blocks[0])])

        self.refinement = nn.Sequential(
            *[Block(n_embd=int(dim * 2 ** 1), n_layer=num_refinement_blocks, layer_id=i) for i in
